...
class Product(models.Model):
    created_user = models.ForeignKey(User, verbose_name="원작자", on_delete=models.CASCADE)
    owner_user = models.ForeignKey(User, verbose_name="소유자", on_delete=models.CASCADE)
    img_path = models.FileField("작품경로", upload_to="media/img")
    img_shape = models.ForeignKey(ImageShape,verbose_name="작품형태",  on_delete=models.SET_NULL)
    category = models.ForeignKey(Category,verbose_name="카테고리",  on_delete=models.SET_NULL)
    title = models.CharField("작품명", max_lenth=50)
    discription = models.TextField("작품 설명", max_lenth=1000)
